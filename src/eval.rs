...
fn assign(
    ...
    match type_.clone() {
        Types::Composite(h) => {
            ...
            let temp_types = type_check_comp(children, vars, local_vars, types, temp_types, expr, final_type, &rhs, &name)?;
...
pub fn eval(
    ...
    Ok(match &expr.0 {
        ...
        Expr::DeclarePrim(name, type_, then) => {
            declare_prim(expr, vars, local_vars, types, name, type_, then)?
        }
